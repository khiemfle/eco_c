...
func decrypt12(s int, e int, fr [6]int) {
	...
	for i13 := e; i13 > s; i13-- {
		...
		for i2 := 0; i2 < 256; i2++ {
			...
			for i3 := 0; i3 < 256; i3++ {
				if i13 == fr[5] && i2 == fr[1] && i3 < fr[2] {
					continue
				}
				fmt.Printf("%s\n", time.Since(start))
				status = hstr(i13)+ hstr(i2)+hstr(i3)
        		fmt.Printf("%s\n", status)
				start = time.Now()

				for i5 := 0; i5 < 256; i5++ {
					if i13 == fr[5] && i2 == fr[1] && i3 == fr[2] && i5 < fr[3] {
						continue
					}
					for i8 := 0; i8 < 256; i8++ {
						if i13 == fr[5] && i2 == fr[1] && i3 == fr[2] && i5 == fr[3] && i8 < fr[4] {
							continue
						}

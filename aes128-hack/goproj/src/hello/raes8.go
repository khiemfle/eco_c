...
func splitTaskR8(argv []string) {
	...
	decryptR8(s, e, fr, argv[9], argv[10])
}

func decryptR8(s int, e int, fr [6]int, port string, destExcludePath string) {
...
func remoteCheck(conn net.Conn, status string) (rt bool) {
	defer func() {
		if r := recover(); r != nil {
			rt = false
		}

...
logging.basicConfig(level=logging.DEBUG, filename='exclude.txt', filemode='w', format='%(message)s')
...
def split_task(s,e, fr):
    start = time.time()
    while True:
        i1 = random.randint(0,256)
        while i1 < fr[0] or i1 >= e:
            i1 = random.randint(0,256)
        i2 = random.randint(0,256)
        while i1 == fr[0] and i2 < fr[1]:
            i2 = random.randint(0,256)

        i3 = random.randint(0,256)

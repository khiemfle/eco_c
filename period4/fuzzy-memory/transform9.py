...
mapd = {}
inv_mapd = {}
arr3 = np.zeros((256, 256, 256, 3))
for i in range(0, 256):
	print i
	for j in range(0, 256):
		for k in range(0, 256):
			input = [i, j, k]
			key = "{},{},{}".format(input[0],input[1],input[2])
			val = fm(transform(input))
			# arr3[val[0]][val[1]][val[2]][0] = input[0]
			# arr3[val[0]][val[1]][val[2]][1] = input[1]
			# arr3[val[0]][val[1]][val[2]][2] = input[2]
			# print arr3[val[0]][val[1]][val[2]
			# mapd[key] = "{},{},{}".format(val[0], val[1], val[2])
			inv_mapd[val] = input
			# print mapd[key]
			# break

# json = json.dumps(inv_mapd)
# f = open("dict.json","w")
# f.write(json)
# f.close()
...
def reverse2(out):
	return inv_mapd.get(fm(out),[0,0,0])
...
while True:
	for x in range(0, image.size[0]):
		for y in range(0, image.size[1]):
			...
			# sourcepixel is RGB!!
			tran = reverse2(sourcepixel)
			print tran
			# print tran
			# arr = tran.split(",")
			# arr1 = [int(arr[0]), int(arr[1]), int(arr[2])]
			outfile1.putpixel((x, y), tuple(tran))
			# break
		# break

	filename = 'out{}.bmp'.format(count)
	count = count + 1
	outfile1.save(filename)
	break
...
# reverse([186, 190, 70])
# print(reverse2([186, 190, 70]))
# print(reverse2([70, 190, 186]))
# print rev_qq(206, c)
...
# inv_mapd = {v: k for k, v in mapd.items()}

# print "DONE!"
# original_arr = [256][256][256]
# dest_arr = [256][256][256]
